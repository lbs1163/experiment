...
void readLSFData(int mdl) {//mdl = model number
	...
	string baseFilename = temp.substr(0, pos3) + "\\resources\\textures\\LSF\\";
	string baseFoldername = temp.substr(0, pos3) + "\\resources\\textures\\LSF";
	string myFilename;

	std::string mdl_str = std::to_string(mdl);

	myFilename = baseFilename + mdl_str + ".txt"; // get full filename of model file

	cout << myFilename << endl;
	
	FILE * fail = fopen(myFilename.c_str(), "r");
